...
impl BurnRateCalculator {
    ...
    fn get_hourly_usage(&self, since: DateTime<Local>) -> HashMap<String, HourlyUsage> {
        ...
        // For simplicity, we'll distribute daily usage evenly across 24 hours
        // In a real implementation, this could use more granular data
        for (date, usage) in &self.daily_usage {
            if *date >= since_date {
                let tokens_per_hour = usage.total_tokens() / 24;
                let cost_per_hour = usage.total_cost / 24.0;
